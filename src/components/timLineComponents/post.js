        ...
        return (
            <div>
                {this.props.posts.map(post => (
                    <Link to = {`/post/singlepost/${post._id}`}>
                        <div className="contnt_2">
                            <div className="div_a">
                                ...
                                <div className="div_top">
                                    <div className="div_top_lft"><img src="images/img_6.png" />{post.postedBy.username}</div>
                                    <div className="div_top_rgt"><span className="span_date">{new Date(post.time).toString().slice(0, 15)}</span><span className="span_time">{new Date(post.time).toString().slice(17, 24)}</span></div>
                                </div>
                ...
                {this.props.page > 1 && <button style={{ marginTop: 30, marginLeft: 170 }} onClick={this.handlePaginationPrevious}>Previous</button>}
                {this.props.posts.length == 10 && <button style={{ marginTop: 30, marginLeft: 170 }} onClick={this.props.handlePagination}>Next</button>}
            </div>
        )
    }
}
